...
import (
	...
	"github.com/XLESSGo/XLESS/extras/transport/udphop"
)
...
type clientConfig struct {
	...
	Obfs          clientConfigObfs      `mapstructure:"obfs"` // 修改这里，使用新的 clientConfigObfs
...
// clientConfigObfs 结构体现在直接嵌入 obfs.ObfuscatorConfig
// 使用 `mapstructure:",squash"` 标签使得 obfs.ObfuscatorConfig 的字段直接暴露在 clientConfigObfs 层面
...
func (c *clientConfig) URI() string {
	q := url.Values{}
	switch strings.ToLower(c.Obfs.Type) { // 直接访问嵌入的 Type 字段
	case "salamander":
		q.Set("obfs", "salamander")
		q.Set("obfs-password", c.Obfs.Password) // 直接访问嵌入的 Password 字段
	}
	if c.TLS.SNI != "" {
		q.Set("sni", c.TLS.SNI)
	}
...
func (c *clientConfig) parseURI() bool {
	...
	if u.Scheme != "xless" && u.Scheme != "hy2" {
		return false
	}
	if u.User != nil {
	...
	if obfsType := q.Get("obfs"); obfsType != "" {
		c.Obfs.Type = obfsType // 直接设置嵌入的 Type 字段
		switch strings.ToLower(obfsType) {
		case "salamander":
			c.Obfs.Password = q.Get("obfs-password") // 直接设置嵌入的 Password 字段
		}
	}
...
// Config validates the fields and returns a ready-to-use xless client config
...
func clientTUN(config tunConfig, c client.Client) error {
	...
	// Helper to convert []string to []string after validation
	parsePrefixes := func(field string, ss []string) ([]string, error) {
		var prefixes []string
		for i, s := range ss {
			var p string
			if strings.Contains(s, "/") {
				...
				p = netip.PrefixFrom(pa, pa.BitLen()).String()
			}
	...
	server := &tun.Server{
		...
		MTU:          config.MTU,
		Timeout:      timeout,
		Inet4Address: []string{prefix4.String()},
		Inet6Address: []string{prefix6.String()},
	}
